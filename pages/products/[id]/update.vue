...
export default {
    ...
    methods: {
        async updateProduct() {
            try {
                const runtimeConfig = useRuntimeConfig();
                const response = await $fetch(runtimeConfig.public.baseUrl + '/products/' + this.$route.params.id, {
                    method: 'PUT',
                    body: this.product
                });
        ...
        <form @submit.prevent="updateProduct">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea v-model="product.description" id="description" name="description" rows="3" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required></textarea>
                </div>
                <div class="sm:col-span-2">
